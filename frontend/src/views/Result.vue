...
<div class="page-header">
  ...
  <a-space size="middle">
    ...
    <a-button
      v-else-if="!agentResponse && editMode"
      type="primary"
      @click="saveChanges"
    >
      💾 保存修改
...
>
  ...
  >
    ...
    >
      <a-tag
        v-if="agentResponse.status === 'waiting_human'"
        color="orange"
      >
        等待确认
      </a-tag>
      ...
      <a-tag
        v-else-if="agentResponse.status === 'completed'"
        color="green"
      >
        已确认完成
      </a-tag>
  ...
  v-if="
    lastAgentFeedback &&
    agentResponse.plan_version > 0
  "
  message="本轮修改要求"
  :description="lastAgentFeedback"
  type="warning"
  show-icon
  style="margin-bottom: 16px"
/>
    ...
    >
      <p style="margin-bottom: 12px">
        AI 已生成新的行程版本，请检查下面的安排。
        确认无误后可以直接批准；如果还有想调整的地方，
        直接告诉 AI 要修改哪一天或哪些内容即可。
      </p>
      ...
      >
        ...
        <a-textarea
          v-model:value="agentFeedback"
          :rows="4"
          placeholder="例如：第二天下午太累，减少一个景点，第一天不要改。"
...
>
  <a-alert
    message="该版本已经通过人工确认。"
    description="本次 Stateful Agent 工作流已完成。如有新的要求，可以继续让 AI 增量修改当前行程。"
    type="success"
    show-icon
    style="margin-bottom: 16px"
  />
  ...
  <a-textarea
    v-model:value="agentFeedback"
    :rows="4"
    placeholder="例如：第二天八达岭太远，换成市区历史文化景点；第一天不要修改。"
    :disabled="agentSubmitting"
  />
  ...
  >
    ...
    >
      <div
        v-if="agentResponse.violations?.length"
        style="margin-top: 12px"
      >
        <strong>未解决问题：</strong>
        ...
        <ul>
          <li
            v-for="(violation, index) in agentResponse.violations"
            :key="index"
          >
            {{ violation.message || violation.code || '未知约束问题' }}
          </li>
        </ul>
      </div>
    </template>
...
<div>
  行程版本：
  {{ agentResponse.plan_version }}
</div>

<div>
  自动重规划次数：
  {{ agentResponse.revision_count }}
</div>
...
const agentResponse =
  ref<AgentRunResponse | null>(null)

const agentFeedback = ref('')

const lastAgentFeedback = ref('')
...
onMounted(async () => {
  const savedFeedback =
    sessionStorage.getItem(
      'tripAgentLastFeedback'
    )

  if (savedFeedback) {
    lastAgentFeedback.value =
      savedFeedback
    }
  const agentData =
    sessionStorage.getItem(
  ...
  if (agentData) {
    try {
      const parsed:
        AgentRunResponse =
          JSON.parse(agentData)

      agentResponse.value = parsed

      if (parsed.data) {
        tripPlan.value = parsed.data
      }
    } catch (error) {
      console.error(
        '解析 Agent Response 失败:',
        error
      )
    }
  }

  // 兼容旧接口 / 旧 sessionStorage
  if (!tripPlan.value) {
    const legacyData =
      sessionStorage.getItem(
        'tripPlan'
...
const applyAgentResponse = async (
...
if (
  response.status === 'waiting_human' ||
  response.status === 'completed'
) {
  lastAgentFeedback.value = feedback
