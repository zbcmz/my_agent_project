...
import { generateStatefulTripPlan } from '@/services/api'
import type {
  TripFormData,
  StatefulPlanRequest,
  AgentRunResponse
...
const USER_ID_STORAGE_KEY = 'travelAgentUserId'
const AGENT_RESPONSE_STORAGE_KEY = 'tripAgentResponse'
const TRIP_PLAN_STORAGE_KEY = 'tripPlan'
const THREAD_ID_STORAGE_KEY = 'tripThreadId'

// ------------------------------------------------------------
// User Identity
// user_id 跨旅行保存，用于 Long-term Memory。
// thread_id 不在这里生成，每次新规划由后端生成。
// ------------------------------------------------------------
...
// Loading UI
//
// Stateful Agent 当前不是 SSE，因此这里的进度只是 UI 反馈，
// 不代表后端真实节点执行百分比。
// ------------------------------------------------------------
...
const startLoadingProgress = () => {
  ...
  loadingStatus.value =
    'AI Agent 正在研究景点、天气、住宿并生成行程...'

  loadingTimer = window.setInterval(() => {
    if (loadingProgress.value < 88) {
      loadingProgress.value +=
        loadingProgress.value < 50 ? 4 : 2
    }
  }, 1500)
}
...
const saveAgentResponse = (
  ...
  // 完整工作流状态：
  // status / thread_id / interrupt / violations / revision_count / plan_version
  sessionStorage.setItem(
    AGENT_RESPONSE_STORAGE_KEY,
  ...
  // 暂时继续保留旧 tripPlan key，
  // 让尚未升级的 Result.vue 仍然能够正常展示。
  if (response.data) {
    sessionStorage.setItem(
...
const handleSubmit = async () => {
  ...
  try {
    const requestData: TripFormData = {
      city: formData.city,
      start_date:
        formData.start_date.format(
          'YYYY-MM-DD'
        ),
      end_date:
        formData.end_date.format(
          'YYYY-MM-DD'
        ),
    ...
    const payload: StatefulPlanRequest = {
      request: requestData,
    ...
    loadingStatus.value =
      '正在执行 Stateful Travel Agent 工作流...'

    const response =
      await generateStatefulTripPlan(payload)
    ...
    // failed 但仍可能带有 best-effort plan，
    // 这种情况也进入结果页，让用户看到 violations。
    if (
